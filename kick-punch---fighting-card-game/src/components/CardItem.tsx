  ...
  return (
    <div
      ...
      className={`relative select-none group rounded-xl border-2 transition-all duration-200 flex flex-col justify-between overflow-hidden cursor-pointer ${sizeClasses} ${
        isSelected
          ? 'ring-4 ring-amber-400 -translate-y-2.5 shadow-2xl shadow-amber-500/40 border-amber-400 bg-zinc-900'
          : isDisabled
          ? 'opacity-40 grayscale cursor-not-allowed border-zinc-800 bg-zinc-950'
          : 'border-zinc-700 hover:border-amber-400/80 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-amber-500/20 bg-zinc-900'
      }`}
      style={{
        boxShadow: isSelected ? `0 0 22px ${fighterColor}77` : undefined
      }}
    >
      ...
      <div>
        <div className="flex items-center justify-between gap-1 mb-1">
          ...
          <div className="flex items-center gap-1">
            {card.moveNumber && (
              <span className="font-arcade font-bold text-[11px] px-1.5 py-0.5 rounded bg-zinc-800 text-amber-400 border border-zinc-700">
                #{card.moveNumber}
              </span>
            )}
            <span className={`text-[9px] sm:text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border ${getCategoryColor()}`}>
              {card.category}
            </span>
          </div>
          ...
          {showCost && card.superCost ? (
            <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-purple-500/30 border border-purple-400 text-purple-200 font-bold text-[10px]">
            ...
            <div 
              ...
              className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-amber-500/20 border border-amber-500/50 text-amber-300 font-bold font-display text-[10px]"
            >
        ...
        <div className="flex items-center justify-between gap-1 mt-0.5 mb-1.5">
          <h4 className="font-arcade text-sm sm:text-base font-bold tracking-wide text-zinc-100 uppercase truncate">
            {card.moveNumber ? `${card.moveNumber}. ` : ''}{card.name}
          </h4>
          <div className="px-1.5 py-0.5 rounded bg-red-950/80 border border-red-800/80 text-red-300 font-bold text-[10px] font-mono whitespace-nowrap">
            PWR: {displayPower}
          </div>
        </div>
      </div>
      ...
      <div className="relative w-full flex-1 rounded-lg overflow-hidden border border-zinc-700/80 bg-zinc-950 flex items-center justify-center my-1 group-hover:border-amber-400/50 transition-colors">
        {card.cardImage ? (
          <img
            src={card.cardImage}
            alt={card.name}
          ...
          <div className="flex flex-col items-center justify-center p-2 text-center">
            ...
            <span className="font-arcade text-xs text-zinc-400">{card.name}</span>
          </div>
        )}

        {/* Floating Power Pill on bottom-right of image */}
        <div className="absolute bottom-1 right-1 bg-zinc-950/90 backdrop-blur-sm border border-amber-500/60 rounded px-1.5 py-0.5 text-[9px] font-bold font-arcade text-amber-400 shadow-md">
          PWR {displayPower}
        </div>
      </div>
      ...
      {(showMatrixBreakdown || size === 'lg') && card.beats && (
        <div className="my-1 space-y-0.5 text-[9px] leading-tight">
          <div className="flex items-center gap-1 text-emerald-400 truncate">
            <Check className="w-2.5 h-2.5 text-emerald-400 shrink-0" />
            <span className="font-semibold shrink-0">BEATS:</span>
          ...
          {card.losesTo && (
            <div className="flex items-center gap-1 text-red-400 truncate">
              ...
              <span className="font-semibold shrink-0">LOSES:</span>
        ...
        {card.effectTags && card.effectTags.length > 0 && size !== 'sm' && !showMatrixBreakdown && (
          <div className="mt-1 flex flex-wrap gap-1">
            {card.effectTags.slice(0, 2).map((tag, idx) => (
              <span key={idx} className="text-[8px] sm:text-[9px] bg-zinc-800/90 text-amber-300 px-1 py-0.5 rounded border border-zinc-700 truncate max-w-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      {card.type === 'finisher' && (
        <div className="absolute inset-0 border-2 border-amber-400/40 rounded-xl pointer-events-none animate-pulse" />
      )}
    </div>
  );
};
